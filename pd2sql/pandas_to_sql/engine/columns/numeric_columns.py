...
def __floordiv__(self, r):
    ...
    return FloatColumn(sql_string=f'( ROUND(({value_to_sql_string(self)} / NULLIF({value_to_sql_string(r)},0)) - 0.5) )')

def __rfloordiv__(self, l):
    # http://sqlite.1065341.n5.nabble.com/floor-help-td46158.html
    return FloatColumn(sql_string=f'( ROUND(({value_to_sql_string(l)} / NULLIF({value_to_sql_string(self)},0)) - 0.5) )')
...
def __truediv__(self, r):
    return FloatColumn(sql_string=f'(({value_to_sql_string(self)} + 0.0) / NULLIF({value_to_sql_string(r)},0))')  

def __rtruediv__(self, l):
    return FloatColumn(sql_string=f'(({value_to_sql_string(l)} + 0.0) / NULLIF({value_to_sql_string(self)},0))')  
