...
def csv2tb(csvpath: str, tbname: str, conn, use_quote:bool = True, pgml_path=""):
    ...
    # Set lock timeout to avoid hanging on concurrent table access
    cursor = conn.cursor()
    ...
    while True:
        try:
            ...
            if pgml_path != "":
                csvpath = pgml_path
            # Use client-side COPY (copy_expert) so it works across Docker containers
            # Server-side COPY FROM would look for the file on the DB server filesystem
            print("copy " + tbname + " from '" + csvpath +
                "' with csv header delimiter ','  encoding 'UTF8'")
            with open(csvpath, 'r', encoding='UTF8') as f:
                cursor.copy_expert(
                    "COPY " + tbname + " FROM STDIN WITH CSV HEADER DELIMITER ',' ENCODING 'UTF8'",
                    f
                )
            conn.commit()
            break
            ...
            if match is not None:
                ...
                # Not a column type error — re-raise
                raise
            conn.rollback()
            # break
