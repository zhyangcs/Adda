...
PG_FUNCTION_INFO_V1(xgboost_regress);
// PG_FUNCTION_INFO_V1(xgboost_classify);
// PG_FUNCTION_INFO_V1(xgboost_predict_classification);
PG_FUNCTION_INFO_V1(xgboost_predict_regression);
// PG_FUNCTION_INFO_V1(xgboost_predict_regression1);
/*
    ...
    output: str of model
*/
Datum xgboost_regress(PG_FUNCTION_ARGS){
    ...
    int ret = XGDMatrixCreateFromMat(
        train_features, arrayLength, num_of_feature, -100, &train_data);
    ...
    // 设置标签
    XGDMatrixSetFloatInfo(train_data, "label", train_labels, arrayLength);

    auto t2 = std::chrono::high_resolution_clock::now();

    // 创建 Booster
    BoosterHandle booster;
    DMatrixHandle eval_dmats[1] = {train_data};
    ...
    for (int i = 0; i < num_iterations; ++i) {
        ...
        XGBoosterUpdateOneIter(booster, i, train_data);
    }
    ...
    bst_ulong out_len{0};
    char const *model_str = (char*) palloc(buffer_len * sizeof(char));
    XGBoosterSaveModelToBuffer(booster, R"({"format": "json"})", &out_len, &model_str);
    elog(WARNING, "model string %ld %s", out_len, model_str);

    // model_map["flight_pipe"] = model_str;



    // elog(WARNING, "model map size %u", model_map.size());

    // BoosterHandle newbooster;
    // XGBoosterCreate(NULL, 0, &newbooster);
    // XGBoosterLoadModelFromBuffer(newbooster, model_str, out_len);
    // elog(WARNING, "load success");
    
    // bst_ulong load_len{0};
    // char const *model_str1 = (char*) palloc(buffer_len * sizeof(char));
    // XGBoosterSaveModelToBuffer(newbooster, R"({"format": "json"})", &load_len, &model_str1);
    // elog(WARNING, "length %ld %ld", out_len, load_len);
...
// input: array[id, xxxx(other features)]
// output: array[id, result]
Datum xgboost_predict_regression(PG_FUNCTION_ARGS){
    // convert the input data to the format that lightgbm can accept
    ArrayType *inputArray = PG_GETARG_ARRAYTYPE_P(0);
    int arrayLength;
    Datum *datums;
    ...
    if(model_map.find(model_name_str_c) != model_map.end()){
        ...
        text *model_str = PG_GETARG_TEXT_P(2);
        const char *model_str_c = text_to_cstring(model_str);
        int iter_num;
        bst_ulong out_len{171150};
        // elog(WARNING, "start load model %d %s %ld", getpid(), model_str_c, out_len);
        auto ret = XGBoosterLoadModelFromBuffer(booster, model_str_c, out_len);
        if(ret){
            elog(WARNING, "Error: LGBM_BoosterLoadModelFromString()\n");
            PG_RETURN_NULL();
        } 
    ...
    auto t2 = std::chrono::high_resolution_clock::now();
    // elog(WARNING, "[%d] load model %f", pid, std::chrono::duration<double>(t2-t1).count());

    int64_t out_len = 0;
    char const config[] = "{\"training\": false, \"type\": 0, "
      "\"iteration_begin\": 0, \"iteration_end\": 0, \"strict_shape\": false}";
    DMatrixHandle predict_feature;
    XGDMatrixCreateFromMat(train_features, arrayLength, num_of_feature, -100, &predict_feature);
    uint64_t const* out_shape;
    uint64_t out_dim;
    float const* result = (float *)palloc(sizeof(float)*arrayLength);
    auto p1 = std::chrono::high_resolution_clock::now();
    auto ret = XGBoosterPredictFromDMatrix(booster, predict_feature, config, &out_shape, &out_dim, &result);
    ...
    auto t3 = std::chrono::high_resolution_clock::now();
    // elog(WARNING, "[%d] predict %f", pid, std::chrono::duration<double>(p2-p1).count());
    ...
    // elog(WARNING, "[%d] end %f", pid, std::chrono::duration<double>(t4-t3).count());
    pfree(values);
    pfree(outnulls);
...
// double* load_and_predict(char *model_str_c, int arrayLength, int num_of_feature, float *train_features, int tid){
//     double *result = (double *)palloc(sizeof(double)*arrayLength);
//     // text *model_name_str = PG_GETARG_TEXT_P(1);
//     // char *model_name_str_c = text_to_cstring(model_name_str);
...
//     // result = load_and_predict(model_str_c, arrayLength, num_of_feature, train_features2, 3);
//     thread1.join();
//     // thread2.join();
