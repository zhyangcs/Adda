...
from src.pg.import_table_patch import importTable_df_with_insert  # 导入补丁函数
import sys
import pickle
...
def importTable_with_split_patch(csvpath:str, tbname:str, label:str, conn, use_quote:bool = True, task_type = "classify", pgml=False):
    """
    使用补丁函数的导入表函数
    """
    import pandas as pd
    ...
    if pgml:
        print("pgml is True, the csv file should be the pgml file")
        df = data_preprocess(df)  # 如果需要预处理函数，请取消注释
    print(df.info())
    ...
    # 使用补丁函数导入数据
    importTable_df_with_insert(df_train, tbname + "_train", conn, use_quote, pgml)
    importTable_df_with_insert(df_test, tbname + "_test", conn, use_quote, pgml)
    importTable_df_with_insert(df, tbname, conn, use_quote, pgml)
...
if __name__ == "__main__": 
    ...
    # 导入数据表并分割为训练集和测试集 - 使用补丁函数
    importTable_with_split_patch(os.path.join(dataset_path, task_name, "train_new.csv"), f"{task_name}_src_tb", target_col, get_conn(), False, task_type)
    t_map = {}
