...
class LLMDagConstructor():
    ...
    def init_task_params(self, data_agenda:list[str], data_desc:list[str], target_col:str, tb_name:str = None, csv_path:str = None, do_unfinished:bool = False, task_name:str = None):
        ...
        if not do_unfinished:
            ...
            if tb_name != None:
                ...
                # 读取完整数据集用于分层采样
                self.original_df = pd.read_sql(f"SELECT * FROM {tb_name}", get_conn())
                print(f"Total dataset size: {self.total_dataset_size}")
            # 如果提供了CSV文件路径，从CSV文件读取数据
            elif csv_path != None:
                self.original_df = pd.read_csv(csv_path)
                self.total_dataset_size = len(self.original_df)
            
            # 使用分层采样初始化数据（确保数据一致性）
            df = self.resample_data()
            print(f"Initial stratified sampling: {df.shape} from {self.original_df.shape}")
            ...
            # 输出采样信息
            print(f"Sampling strategy: Stratified sampling enabled, ratio: {self.sample_ratio}, resampling: {'enabled' if self.resample_interval > 0 else 'disabled'}")
    ...
    def resample_data(self):
        ...
        if self.original_df is None:
            print("Warning: No original data available for resampling")
            return None
            
        sample_size = max(1, int(self.total_dataset_size * self.sample_ratio))
        
        if self.use_stratified_sampling and self.target_col in self.original_df.columns:
            # 使用分层采样保持类别分布
            try:
                # 分层采样
                sampled_df = self.original_df.groupby(self.target_col).apply(
                    lambda x: x.sample(n=max(1, int(len(x) * self.sample_ratio)), random_state=42)
                ).reset_index(drop=True)
                
                print(f"Stratified sampling: {sampled_df.shape} from {self.original_df.shape}")
                return sampled_df
            except Exception as e:
                print(f"Stratified sampling failed, falling back to random sampling: {e}")
        
        # 随机采样
        sampled_df = self.original_df.sample(n=min(sample_size, len(self.original_df)), random_state=42)
        print(f"Random sampling: {sampled_df.shape} from {self.original_df.shape}")
        return sampled_df
    
    def update_all_nodes_data(self, new_df):
        """更新所有节点的数据
        ...
        # 检查新数据是否包含目标列
        if self.target_col in new_df.columns:
            # 重新提取和预处理标签数据（修复数据泄露）
            new_label, _ = prepare_df_for_train(pd.DataFrame(new_df[self.target_col]))
            new_df = new_df.drop(columns = [self.target_col])
            
            # 更新根节点数据和标签
            if hasattr(self, 'root'):
                self.root.in_cur_df = new_df.copy()
                self.root.out_cur_df = new_df.copy()
                self.label = new_label  # 同步更新标签
        ...
        for node in self.dag.nodes():
            if node != self.root:
                try:
                    # 重新执行节点代码
                    exec_env = {'df': node.in_cur_df.copy()}
                    # 先执行修复节点
                    for fix_node in node.fixing_node:
                        exec(fix_node.task_code, exec_env)
                    # 执行主节点代码
                    exec(node.task_code, exec_env)
                    node.out_cur_df = exec_env['df']
                    
                    # 重新评估节点
                    self.evaluate_node(node)
                    
                except Exception as e:
                    print(f"Failed to update node {node.node_id}: {e}")
    ...
    def should_resample(self):
        ...
        if self.resample_interval <= 0:
            return False
        return self.use_stratified_sampling and self.step_count > 0 and self.step_count % self.resample_interval == 0
    ...
    def astar_k_step(self, step_num:int, data_agenda:list[str] = None, data_desc:list[str] = None, target_col:str = None, tb_name:str=None, csv_path:str=None, do_unfinished:bool = False, task_name:str = None):
        ...
        for i in range(start_idx, step_num, 1):
            ...
            if self.should_resample():
                print(termcolor.colored(f"Resampling data at step {i} (interval: {self.resample_interval})", "blue"))
                new_df = self.resample_data()
                if new_df is not None:
                    self.update_all_nodes_data(new_df)
    ...
    def get_best_code(self):
        ...
        for idx, node in enumerate(self.output_nodes):
            ...
            with open(code_path, "w") as f:
                ...
                f.write(f"conn = psycopg2.connect(dbname='{pg_db}', user='{pg_user}', password='{pg_password}', host='{pg_host}', port={pg_port})\n")
